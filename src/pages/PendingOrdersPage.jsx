...
export default function PendingOrdersPage({ user, apiUrl, setPage }) {
    ...
    const [isCancelingExpired, setIsCancelingExpired] = useState(false);
    const [linePayAbnormalCount, setLinePayAbnormalCount] = useState(0);
    const [activeDropdownOrderId, setActiveDropdownOrderId] = useState(null);

    const handleCancelExpiredLinePay = async () => {
        if (!window.confirm('確定要一鍵作廢所有「超過 24 小時未付款」的 LINE Pay 訂單嗎？\n(這會將它們轉為已取消，但不會影響庫存)')) return;
        
        setIsCancelingExpired(true);
        try {
            const res = await fetch(getBackendUrl('/api/linepay/cancel-expired'), {
                method: 'POST',
            ...
            if (data.success) {
                alert(data.message || `成功作廢了 ${data.count} 筆逾期訂單！`);
                if (data.count > 0 && typeof fetchOrders === 'function') fetchOrders();
            } else {
                alert('執行失敗：' + (data.error || data.message || '未知錯誤'));
            }
        } catch (err) {
            console.error(err);
            alert('連線異常，請稍後再試');
        } finally {
            setIsCancelingExpired(false);
        }
    };

    const handleSyncLinePayStatus = async (order, e) => {
        if (e) e.stopPropagation();
        setIsSyncingOrderId(order.orderId);
        try {
            // 由於前端 React state 可能還沒更新 note（例如剛下單沒按 F5），
            // 我們直接把 orderId 丟給後端，後端會自動從最新資料庫中提取交易單號！
            let res = await fetch(getBackendUrl('/api/linepay/sync-status'), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ orderId: order.orderId, transactionId: '' })
            });
            let data = await res.json();
            
            // 如果後端在資料庫也找不到，就會回傳 requireManualInput，這時才要求客服手動輸入
            if (data.requireManualInput) {
                const manualTxId = prompt('無法自動從資料庫取得交易序號，請手動輸入客人收據上的 LINE Pay 20 碼交易序號：');
                if (!manualTxId) {
                    setIsSyncingOrderId(null);
                    return;
                }
                res = await fetch(getBackendUrl('/api/linepay/sync-status'), {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ orderId: order.orderId, transactionId: manualTxId.trim() })
                });
                data = await res.json();
            }

            if (data.success) {
                alert(data.message || '對帳成功！訂單已更新為「已付款」');
                fetchOrders(); // 重新整理列表
            } else {
                alert('對帳失敗：' + (data.message || '無法取得狀態'));
            }
        } catch (err) {
            console.error(err);
            alert('系統連線錯誤，請稍後再試');
    ...
    const fetchOrders = useCallback(async () => {
        ...
        try {
            ...
            // 同步獲取 LINE Pay 異常數量
            try {
                const countRes = await fetch(getBackendUrl('/api/linepay/abnormal-count'), {
                    headers: { 'Content-Type': 'application/json' }
                });
                const countData = await countRes.json();
                if (countData && countData.success) {
                    setLinePayAbnormalCount(countData.count || 0);
                }
            } catch (err) {
                console.error('Failed to fetch abnormal count:', err);
            }
        } catch (error) {
    ...
    // 安全剪貼簿複製函式 (DOM 傳統選取技術，100% 繞過 HTTP 與非同步權限阻擋)
    ...
    const handleConfirmGenerateLinePayLink = async () => {
        if (!linePayConfigModal) return;
        const { orderId, customerName, customAmount, remark } = linePayConfigModal;
        const amount = Number(customAmount);
        if (isNaN(amount) || amount <= 0) {
            alert('請輸入有效的補繳金額（必須大於 0）');
            return;
        }

        setGeneratingOrderId(orderId);
        try {
            const productName = remark ? `米立微訂單 #${orderId} (${remark})` : `米立微訂單 #${orderId}`;
            const res = await fetch(getBackendUrl('/api/linepay/request'), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    orderId,
            ...
            setLinePayConfigModal(null);

            if (data.success && data.paymentUrl) {
                const remarkText = remark ? `\n💡 補繳說明｜${remark}` : '';
                const formattedMsg = `${customerName} 您好～感謝您的訂購😊\n\n以下為『米立微MilkZeroWaste』\n提供的 LINE Pay 付款資訊：\n\n📋 訂單編號｜#${orderId}${remarkText}\n💰 應付金額｜$${amount} 元\n\n🔗 LINE Pay 付款連結\n${data.paymentUrl}\n\n⚠️ 提醒您：此付款連結僅可點擊一次。\n\n付款成功後，系統將自動完成對帳，無需另外回傳付款資訊。`;

                copyTextSafely(formattedMsg);

                setLinePayPayLinkModal({
                    orderId,
                    amount,
                    customerName,
                    text: formattedMsg,
                ...
                alert('產生 LINE Pay 補繳連結失敗：' + (data.message || '請稍後再試'));
            }
        } catch (err) {
            setGeneratingOrderId(null);
            console.error('Failed to generate LINE Pay link:', err);
            alert('請求失敗：' + (err?.message || '請檢查網路連線'));
        }
    };

    const handleOpenLinePayRefund = (order, e) => {
        if (e) {
            e.preventDefault();
            e.stopPropagation();
        }
        let txId = '';
        if (order.note && order.note.includes('交易單號: ')) {
            const match = order.note.match(/交易單號:\s*(\d+)/);
            if (match) txId = match[1];
        }

        const totalAmt = Number(order.totalAmount || order.grandTotal || 0);
        
        // 解析前端歷史退款金額
        let previousRefunded = 0;
        if (order.note) {
            const refundMatches = order.note.matchAll(/\[LINE Pay 退款紀錄.*?\] 金額: \$(\d+(\.\d+)?) 元/g);
            for (const match of refundMatches) {
                previousRefunded += Number(match[1]);
            }
        }
        const remainingRefundable = Math.max(0, totalAmt - previousRefunded);

        setLinePayRefundModal({
            orderId: order.orderId,
            customerName: order.customerName || order.recipientName || '顧客',
            totalAmount: totalAmt,
            previousRefunded: previousRefunded,
            remainingRefundable: remainingRefundable,
            transactionId: txId,
            refundType: 'full',
            customRefundAmount: String(remainingRefundable),
            refundReason: ''
        });
    };

    const handleConfirmLinePayRefund = async () => {
        if (!linePayRefundModal) return;
        const { orderId, transactionId, refundType, customRefundAmount, refundReason, remainingRefundable } = linePayRefundModal;

        let numRefundAmt;
        if (refundType === 'partial') {
            numRefundAmt = Number(customRefundAmount);
            if (!numRefundAmt || numRefundAmt <= 0) {
                alert('請輸入有效的退款金額（需大於 0）');
                return;
            }
            if (numRefundAmt > remainingRefundable) {
                alert(`退款金額 $${numRefundAmt} 不能超過剩餘可退餘額 $${remainingRefundable}`);
                return;
            }
        }
        ...
        try {
            const res = await fetch(getBackendUrl('/api/linepay/refund'), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    orderId,
                    transactionId,
                    refundAmount: refundType === 'full' ? undefined : numRefundAmt,
                    refundReason
                })
            });
            ...
            if (data.success) {
                ...
                alert(`❌ 退款失敗 (${data.returnCode || 'Error'}): ${data.message || JSON.stringify(data)}`);
            }
        } catch (err) {
    ...
    return (
        <div className="max-w-6xl mx-auto min-h-screen flex flex-col p-4 gap-4">
            ...
            <div className="bg-[var(--bg-secondary)] p-4 sm:p-5 rounded-2xl border border-[var(--border-primary)] shadow-sm space-y-3.5 relative z-30 overflow-visible">
                ...
                <div className="flex items-center justify-between gap-4">
                    ...
                    <div className="flex gap-2">
                        {(activeTab === 'PENDING' || activeTab === 'LINEPAY_ABNORMAL') && (
                            <button
                                onClick={handleCancelExpiredLinePay}
                                disabled={isCancelingExpired}
                                className="p-2 bg-red-50 text-red-600 hover:bg-red-100 rounded-xl border border-red-200 transition-all shadow-2xs flex items-center gap-1.5 text-xs font-bold disabled:opacity-50"
                                title="一鍵作廢超過 24 小時未付款的 LINE Pay 訂單 (不影響庫存)"
                            >
                                {isCancelingExpired ? <RefreshCw size={16} className="animate-spin" /> : <Trash2 size={16} />}
                                <span className="hidden sm:inline">作廢逾期 LINE Pay</span>
                            </button>
                        )}
                        <button
                            onClick={fetchOrders}
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between border-b border-[var(--border-primary)] pb-1.5 gap-2 w-full max-w-full">
                <div className="grid grid-cols-2 lg:flex lg:w-auto w-full items-center gap-1 sm:gap-2">
                    <button
                    ...
                    <button
                        onClick={() => handleTabChange('LINEPAY_ABNORMAL')}
                        className={`px-1 sm:px-4 py-2 font-bold text-xs sm:text-sm text-center transition-colors border-b-2 whitespace-nowrap flex items-center justify-center gap-1.5 ${activeTab === 'LINEPAY_ABNORMAL'
                            ? 'border-red-500 text-red-600'
                            : 'border-transparent text-[var(--text-secondary)] hover:text-[var(--text-primary)]'
                            }`}
                    >
                        <span>LINE Pay 異常</span><span className="hidden md:inline text-[11px] opacity-80"> (UNPAID)</span>
                        {linePayAbnormalCount > 0 && (
                            <span className="bg-red-500 text-white text-[10px] px-1.5 py-0.5 rounded-full font-black shadow-sm">
                                {linePayAbnormalCount}
                            </span>
                        )}
                    </button>
                </div>
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center justify-between gap-3 flex-wrap">
                                            ...
                                            <div className="flex flex-col items-end shrink-0">
                                                ...
                                                {(!(order.paymentStatus === '已付款' || order.paymentStatus === '已請款' || order.paymentStatus === '已入帳' || (order.paymentStatus && order.paymentStatus.includes('部分退款')) || order.paymentStatus === 'off') || (order.paymentMethod === 'LINE Pay' || (order.note && order.note.includes('LINE Pay')))) && (
                                                    <div className="relative inline-flex items-center h-4 mt-1">
                                                        <button
                                                            type="button"
                                                            onClick={(e) => {
                                                                e.preventDefault();
                                                                e.stopPropagation();
                                                                setActiveDropdownOrderId(activeDropdownOrderId === order.orderId ? null : order.orderId);
                                                            }}
                                                            className="text-gray-300 hover:text-gray-500 rounded-md transition-colors cursor-pointer p-2 -m-2"
                                                            title="更多操作"
                                                        >
                                                            <ChevronDown size={20} strokeWidth={3} />
                                                        </button>

                                                        {activeDropdownOrderId === order.orderId && (
                                                            <>
                                                                    ...
                                                                    {!(order.paymentStatus === '已全額退款' || (order.paymentStatus && order.paymentStatus.includes('全額退款'))) && (
                                                                        <button
                                                                            ...
                                                                            onClick={(e) => {
                                                                                ...
                                                                                e.stopPropagation();
                                                                                setActiveDropdownOrderId(null);
                                                                                handleOpenLinePayConfig(order, e);
                                                                            }}
                                                                            className="w-full text-left px-4 py-2.5 text-sm font-bold text-gray-700 hover:bg-gray-50 flex items-center gap-2 disabled:opacity-60 cursor-pointer"
                                                                        >
                                                                            {generatingOrderId === order.orderId ? (
                                                                                <>
                                                                                    <div className="w-3.5 h-3.5 border-2 border-gray-600 border-t-transparent rounded-full animate-spin shrink-0" />
                                                                                    <span>產生中...</span>
                                                                                </>
                                                                            ) : (
                                                                                <>
                                                                    ...
                                                                    {(order.paymentStatus === '已付款' || order.paymentStatus === '已請款' || order.paymentStatus === '已入帳' || (order.paymentStatus && order.paymentStatus.includes('部分退款')) || order.paymentStatus === 'off') && (
                                                                        <button
                                                                            ...
                                                                            onClick={(e) => {
                                                                                ...
                                                                                e.stopPropagation();
                                                                                setActiveDropdownOrderId(null);
                                                                                handleOpenLinePayRefund(order, e);
                                                                            }}
                                                                            className="w-full text-left px-4 py-2.5 text-sm font-bold text-rose-600 hover:bg-rose-50 flex items-center gap-2 cursor-pointer"
                                                                        >
                                                                            <span>💸</span>
                                                                            <span>LINE Pay 申請退款</span>
                                                                        </button>
                                                                    )}
                                                                </div>
                                                            </>
                                        ...
                                        <div className="flex items-center gap-2 flex-wrap text-xs pt-0.5">
                                            ...
                                            {/* 付款狀態與 LINE Pay 補繳按鈕 */}
                                            {order.paymentStatus === '已全額退款' || (order.paymentStatus && order.paymentStatus.includes('全額退款')) ? (
                                                <span className="inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-lg font-bold bg-slate-100 text-slate-700 border border-slate-300 shadow-2xs">
                                                    🚫 此筆訂單全額退款
                                                </span>
                                            ) : (order.paymentStatus === '已付款' || order.paymentStatus === '已請款' || order.paymentStatus === '已入帳' || (order.paymentStatus && order.paymentStatus.includes('部分退款')) || order.paymentStatus === 'off') ? (
                                                <div className="flex items-center gap-1.5">
                                                    <span className={`inline-flex items-center gap-1 text-xs px-2.5 py-0.5 rounded font-bold border ${
                                                        order.paymentStatus && order.paymentStatus.includes('部分退款')
                                                            ? 'bg-blue-50 text-blue-700 border-blue-300'
                                                            : 'bg-emerald-50 text-emerald-600 border-emerald-200'
                                                    }`}>
                                                        ✓ {order.paymentStatus && order.paymentStatus.includes('部分退款') ? order.paymentStatus : '已付款'}
                                                    </span>
                                                    {/* LINE Pay 退款已移至右上角更多操作選單中 */}
                                                </div>
                                            ) : (
                                                <div className="flex items-center gap-1.5">
                                                    <button
                                                    ...
                                                    {/* 新增的同步對帳按鈕 */}
                                                    {(order.paymentMethod === 'LINE Pay' || (order.note && order.note.includes('LINE Pay'))) && (
                                                        <button
                                                            type="button"
                                                            disabled={isSyncingOrderId === order.orderId}
                                                            onClick={(e) => handleSyncLinePayStatus(order, e)}
                                                            className="text-xs px-2.5 py-1 font-extrabold rounded-lg bg-orange-50 hover:bg-orange-100 text-orange-600 border border-orange-300 shadow-2xs flex items-center gap-1 transition-all cursor-pointer disabled:opacity-60"
                                                            title="向 LINE 官方主動查詢交易狀態並自動補繳 (防掉單專用)"
                                                        >
                                                            {isSyncingOrderId === order.orderId ? (
                                                                <>
                                                                    <div className="w-3 h-3 border-2 border-orange-600 border-t-transparent rounded-full animate-spin shrink-0" />
                                                                    <span>對帳中...</span>
                                                                </>
                                                            ) : (
                                                                <>
            ...
            {/* 💸 LINE Pay 訂單退款設定 Modal */}
            {linePayRefundModal && (
                <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-[150] p-4">
                    <div className="bg-white border border-rose-100 shadow-2xl rounded-2xl p-6 w-full max-w-md space-y-4 animate-in fade-in zoom-in-95 duration-150">
                        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                            <h3 className="text-base font-bold text-slate-800 flex items-center gap-2">
                                <span className="text-rose-500 font-bold">💸</span> LINE Pay 訂單退款/退費
                            </h3>
                            <button
                                onClick={() => setLinePayRefundModal(null)}
                        ...
                        <div className="space-y-3">
                            <div className="bg-rose-50/50 rounded-xl p-3 border border-rose-100 text-xs space-y-1.5">
                                <div className="flex justify-between text-slate-600">
                                    <span>訂單編號:</span>
                                    <span className="font-mono font-bold text-slate-800">#{linePayRefundModal.orderId}</span>
                                </div>
                                <div className="flex justify-between text-slate-600">
                                    <span>顧客姓名:</span>
                                    <span className="font-bold text-slate-800">{linePayRefundModal.customerName}</span>
                                </div>
                                <div className="flex justify-between text-slate-600">
                                    <span>原扣款總額:</span>
                                    <span className="font-mono font-bold text-rose-700">${linePayRefundModal.totalAmount} 元</span>
                                </div>
                                {linePayRefundModal.previousRefunded > 0 && (
                                    <>
                                        <div className="flex justify-between text-slate-600">
                                            <span>已退金額:</span>
                                            <span className="font-mono font-bold text-blue-600">${linePayRefundModal.previousRefunded} 元</span>
                                        </div>
                                        <div className="flex justify-between text-slate-600 border-t border-rose-100 pt-1.5 mt-1.5">
                                            <span>可退餘額:</span>
                                            <span className="font-mono font-bold text-rose-700">${linePayRefundModal.remainingRefundable} 元</span>
                                        </div>
                                    </>
                                )}
                                {linePayRefundModal.transactionId && (
                                    <div className="flex justify-between text-slate-500 text-[11px] pt-1">
                                        <span>LINE Pay 交易號:</span>
                                        <span className="font-mono">{linePayRefundModal.transactionId}</span>
                                    </div>
                                )}
                            </div>
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-2 gap-2.5">
                                    <button
                                        type="button"
                                        onClick={() => setLinePayRefundModal(prev => prev ? { ...prev, refundType: 'full', customRefundAmount: String(prev.remainingRefundable) } : null)}
                                    ...
                                    >
                                        <span className="text-sm">🔴 退還餘額</span>
                                        <span className="font-mono text-[11px] opacity-90">${linePayRefundModal.remainingRefundable} 元</span>
                                    </button>

                                    <button
                                        type="button"
                                        onClick={() => setLinePayRefundModal(prev => prev ? { ...prev, refundType: 'partial', customRefundAmount: prev.customRefundAmount || '' } : null)}
                                        className={`p-2.5 rounded-xl border text-xs font-bold transition-all flex flex-col items-center gap-1 cursor-pointer ${
                                            linePayRefundModal.refundType === 'partial'
                                                ? 'bg-amber-500 text-white border-amber-600 shadow-md shadow-amber-500/20'
                                                : 'bg-slate-50 text-slate-700 border-slate-200 hover:bg-slate-100'
                                        }`}
                                    >
                                        <span className="text-sm">🟡 部分退款</span>
                                        <span className="text-[11px] opacity-90">自訂指定金額</span>
                                    </button>
                                </div>
                            </div>
                            ...
                            {linePayRefundModal.refundType === 'partial' && (
                                <div className="animate-in fade-in duration-150">
                                    <label className="block text-xs font-bold text-slate-700 mb-1">
                                        💰 部分退款金額 (元) <span className="text-rose-500">*</span>
                                    </label>
                                    <input
                                        type="number"
                                        value={linePayRefundModal.customRefundAmount}
                                        onChange={(e) => setLinePayRefundModal(prev => prev ? { ...prev, customRefundAmount: e.target.value } : null)}
                                        placeholder={`請輸入退款金額（上限 $${linePayRefundModal.totalAmount}）`}
                                        className="w-full px-3.5 py-2.5 bg-white border border-slate-300 rounded-xl text-sm font-bold text-rose-600 focus:outline-none focus:ring-2 focus:ring-rose-500 font-mono"
                                    />
                                </div>
                            )}
                            ...
                            <div>
                                ...
                                <input
                                    type="text"
                                    value={linePayRefundModal.refundReason}
                                    onChange={(e) => setLinePayRefundModal(prev => prev ? { ...prev, refundReason: e.target.value } : null)}
                                    placeholder="例如：商品缺貨退款 / 顧客取消追加品項"
                        ...
                        <div className="flex gap-2 pt-2">
                            <button
                                type="button"
                                onClick={() => setLinePayRefundModal(null)}
                                className="px-4 py-2.5 rounded-xl font-bold text-xs bg-slate-100 text-slate-600 hover:bg-slate-200 cursor-pointer"
                            >
                                取消
                            </button>
                            <button
                                type="button"
                                disabled={refundingOrderId === linePayRefundModal.orderId}
                                onClick={handleConfirmLinePayRefund}
                                className="flex-1 py-2.5 rounded-xl font-bold text-xs bg-rose-600 hover:bg-rose-700 text-white shadow-md shadow-rose-500/20 active:scale-95 transition-all flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-60"
                            >
                                {refundingOrderId === linePayRefundModal.orderId ? (
                                    <>
                                        <div className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" />
                                        <span>LINE Pay 退款處理中...</span>
                                    </>
                                ) : (
                                    <span>🚨 確認向 LINE Pay 官方申請退款</span>
                                )}
                            </button>
                        </div>
                    </div>
            ...
            {linePayConfigModal && (
                <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-[150] p-4">
                    <div className="bg-white border border-slate-200 shadow-2xl rounded-2xl p-6 w-full max-w-md space-y-4 animate-in fade-in zoom-in-95 duration-150">
                        ...
                        <div className="space-y-3">
                            <div className="bg-slate-50 rounded-xl p-3 border border-slate-200/60 text-xs space-y-1">
                                <div className="flex justify-between text-slate-600">
                                    <span>訂單編號:</span>
                                    <span className="font-mono font-bold text-slate-800">#{linePayConfigModal.orderId}</span>
                                </div>
                                <div className="flex justify-between text-slate-600">
                                    <span>顧客姓名:</span>
                                    <span className="font-bold text-slate-800">{linePayConfigModal.customerName}</span>
                                </div>
                            </div>

                            <div>
                                <label className="block text-xs font-bold text-slate-700 mb-1">
                                    💰 補繳金額 (元) <span className="text-pink-500">*</span>
                                </label>
                                <input
                                    type="number"
                                    value={linePayConfigModal.customAmount}
                                    onChange={(e) => setLinePayConfigModal(prev => prev ? { ...prev, customAmount: e.target.value } : null)}
                                    placeholder="請輸入補繳金額，例如 20"
                                    className="w-full px-3.5 py-2.5 bg-white border border-slate-300 rounded-xl text-sm font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                                />
                            </div>
                            ...
                            <div>
                                ...
                                <input
                                    type="text"
                                    value={linePayConfigModal.remark}
                                    onChange={(e) => setLinePayConfigModal(prev => prev ? { ...prev, remark: e.target.value } : null)}
                                    placeholder="例如：追加鮮乳1瓶 / 運費補繳"
                        ...
                        <div className="flex gap-2 pt-2">
                            ...
                            <button
                                type="button"
                                disabled={generatingOrderId === linePayConfigModal.orderId}
                                onClick={handleConfirmGenerateLinePayLink}
                                className="flex-1 py-2.5 rounded-xl font-bold text-xs bg-emerald-500 hover:bg-emerald-600 text-white shadow-md shadow-emerald-500/20 active:scale-95 transition-all flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-60"
                            >
                                {generatingOrderId === linePayConfigModal.orderId ? (
                                    <>
                                        <div className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" />
                                        <span>產生中...</span>
                                    </>
                                ) : (
                                    <span>✨ 產生補繳網址並複製訊息</span>
                                )}
                            </button>
                        </div>
                    </div>
            ...
            {/* 📱 LINE Pay 補繳網址專屬彈窗 Modal */}
            {linePayPayLinkModal && (
                <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-[150] p-4">
                    <div className="bg-white border border-slate-200 shadow-2xl rounded-2xl p-6 w-full max-w-lg space-y-4 animate-in fade-in zoom-in-95 duration-150">
                        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                            <h3 className="text-base font-bold text-slate-800 flex items-center gap-2">
                                <span className="text-emerald-500 font-bold">📱</span> LINE Pay 付款訊息已生成
                            </h3>
                            <button
                                onClick={() => setLinePayPayLinkModal(null)}
                                className="text-slate-400 hover:text-slate-600 p-1 rounded-full hover:bg-slate-100 cursor-pointer"
                            >
                                <X size={18} />
                            </button>
                        ...
                        <div className="text-xs text-slate-500 font-medium">
                            已為 <strong className="text-slate-800">{linePayPayLinkModal.customerName}</strong>（單號 #{linePayPayLinkModal.orderId}）產生專屬付款網址。點擊下方按鈕即可一鍵複製傳給顧客：
                        </div>

                        <textarea
                            readOnly
                            value={linePayPayLinkModal.text}
                            onClick={(e) => e.target.select()}
                            className="w-full h-44 p-3 bg-slate-50 border border-slate-300 rounded-xl text-xs font-mono text-slate-800 leading-relaxed resize-none focus:outline-none focus:ring-2 focus:ring-emerald-500"
                        />
                        ...
                        <div className="flex gap-2">
                            <button
                                ...
                                onClick={(e) => {
                                    ...
                                    copyTextSafely(linePayPayLinkModal.text);
                                    setLinePayPayLinkModal(prev => prev ? { ...prev, isCopied: true } : null);
                                }}
